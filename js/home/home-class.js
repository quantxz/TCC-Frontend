class HomeFunctions {
    ...
    *postsGenerator(postsArray) {
    ...
    async renderMostFamousPosts(data) {
        const section = document.querySelector(".explore-section")
        const postsDiv = section.children[1]   
        ...
        data.forEach((post) => {
            console.log(post)
            postElement.setAttribute('metadata', JSON.stringify({ id: post.id, createdAt: post.createdAt, type: post.type }));
            postElement.setAttribute('data-likes', post.likes || 0);
            const response = this.identifier.Identifier(post.content);
            let content = '';
    
            if (response.type == "link" && response.image == "no") {
                content = `<a href="${post.content}">${post.content}</a>`;
            } else if (response.type == "link" && response.image == "yes") {
                content = `<img src="${post.content}" alt="post ${post.title} image as content">`;
            } else {
            ...
            postElement.innerHTML = `
                <div class="post-header">
                    ...
                    <div class="post-profile-infos">
                        <p>${post.user.name ? post.user.name : "undefined"}</p>
                        <p>@${post.author}</p>
                    </div>
                    <div class="post-options-header">
                    </div>
                ...
                <div class="post-footer">
                    <div class="post-likes" id="UniqueValueIdentifier${post.id}">
                        <div class="heart-container" title="Like">
                            <input type="checkbox" class="checkbox likeCheckbox" id="postInputCheckId${post.id}">
                            <div class="svg-container">
                                <svg viewBox="0 0 24 24" class="svg-outline" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M17.5,1.917a6.4,6.4,0,0,0-5.5,3.3,6.4,6.4,0,0,0-5.5-3.3A6.8,6.8,0,0,0,0,8.967c0,4.547,4.786,9.513,8.8,12.88a4.974,4.974,0,0,0,6.4,0C19.214,18.48,24,13.514,24,8.967A6.8,6.8,0,0,0,17.5,1.917Zm-3.585,18.4a2.973,2.973,0,0,1-3.83,0C4.947,16.006,2,11.87,2,8.967a4.8,4.8,0,0,1,4.5-5.05A4.8,4.8,0,0,1,11,8.967a1,1,0,0,0,2,0,4.8,4.8,0,0,1,4.5-5.05A4.8,4.8,0,0,1,22,8.967C22,11.87,19.053,16.006,13.915,20.313Z"></path>
                                </svg>
                        ...
                        <p class='postLikes postLikeId${post.id} likes${post.likes}' value=${post.likes < 0 ? 0 : post.likes}>${post.likes < 0 ? 0 : post.likes} Curtidas</p>
                        <div class="commentsOfPost post-comments">
                            <img src="/assets/chat-dots-fill.svg">
                            <p>Comentar</p>
                        </div> 
